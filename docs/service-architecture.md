...
이 문서는 현재 로컬 SQLite 기반 `jp-vocab-reader`를 여러 사용자가 함께 쓰는 웹서비스로 확장하기 위한 설계 초안이다. 이번 단계에서는 실제 로그인, DB 스키마 변경, API 동작 변경을 하지 않고, PostgreSQL과 다중 사용자 구조로 전환하기 전에 필요한 방향을 정리한다.
